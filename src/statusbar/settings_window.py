...
- Checkboxes for multi-flag fields (Default Formats)
"""
...
# widget_type: "text" | "password" | "select" | "checkboxes"
...
def show_settings(on_save=None):
    ...
    for section_title, section_fields in SECTIONS:
        ...
        for label_text, key, widget_type, options_or_ph in section_fields:
            ...
            if widget_type == "checkboxes":
                # options_or_ph is [(label, flag_value), ...]
                active_flags = {f.strip().lower()
                                for f in str(cfg.get(key, "")).split(",") if f.strip()}
                cb_width  = FIELD_W // len(options_or_ph)
                checkboxes = []
                for i, (cb_label, flag_val) in enumerate(options_or_ph):
                    cb = NSButton.alloc().initWithFrame_(
                        NSMakeRect(fx + i * cb_width, y, cb_width, ROW_H)
                    )
                    cb.setButtonType_(NSSwitchButton)
                    cb.setTitle_(cb_label)
                    cb.setFont_(FONT)
                    cb.setState_(1 if flag_val.lower() in active_flags else 0)
                    content_view.addSubview_(cb)
                    checkboxes.append((flag_val, cb))
                widgets[key] = checkboxes   # list of (flag_val, NSButton)
